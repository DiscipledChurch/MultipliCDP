    ...
    jasmine.getEnv().addReporter({
      specDone: function(result) {
        ...
        browser.getSession().then(function(session) {
          ...
          var formattedResult = result.failedExpectations[0].message.replace(/'/, "\\'");

            var exec = require('child_process').exec;
            var cmd = 'curl -X PUT -s -d \'{"name": "' + result.fullName + '", "passed": false, "custom-data": { "message": "messed up"} }\' -u ' + process.env.SAUCE_USERNAME + ':' + process.env.SAUCE_ACCESS_KEY + ' https://saucelabs.com/rest/v1/' + process.env.SAUCE_USERNAME + '/jobs/' + session.getId();
...
console.log(result.failedExpectations[0].message);
